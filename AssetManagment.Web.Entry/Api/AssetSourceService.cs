...
namespace AssetManagment.Web.Entry.Api
{
    ...
    public class AssetSourceService
    {
        ...
        public AssetSourceService(ISqlSugarClient db)
        {
            _db = db;
        }


        [HttpPost("/api/asseetsource/add")]
        public ApiResult<string> Add([FromBody] RequestBase data)
        {
            ...
            try
            {
                ...
                AssetSources sources = new AssetSources { Title = data.Title, Code = data.Code, Note = data.Note };
                int i = _db.Queryable<AssetSources>().Where(it => it.Code == sources.Code).Count();
                if (i > 0)
                {
                    throw new UniqueExcetion();
                }
                _db.Insertable<AssetSources>(sources).ExecuteCommand();
                _db.Ado.CommitTran();
            }
        ...
        [HttpPost("/api/asseetsource/update")]
        public ApiResult<string> Update([FromBody] RequestBase data)
        {
            ...
            {
                _db.Ado.BeginTran(System.Data.IsolationLevel.Serializable);
                AssetSources sources = new AssetSources { Id = data.Id, Code = data.Code, Note = data.Note, Title = data.Title };
                var d = _db.Queryable<AssetCategory>().Where(it => it.Code == sources.Code).ToList();
                if (d.Count() == 1 && d.First().Id != data.Id)
                {
                    throw new UniqueExcetion();
                }
                var i = _db.Updateable(sources).WhereColumns(it => new { it.Id }).ExecuteCommand();
        ...
        [HttpPost("/api/asseetsource/remove")]
